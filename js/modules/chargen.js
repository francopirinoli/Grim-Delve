...
renderPrintVersion: async (container) => {
        ...
        // Filter: Spells (Magic Tab)
        const spellList = allFeatures.filter(f => f.isSpell);
        
        // Filter: Recipes (Crafting)
        const recipeList = allFeatures.filter(f => f.isCraftable);
        
        // Filter: Active Abilities (Exploits, Class Actions, but NOT Spells/Recipes)
        const activeList = allFeatures.filter(f => 
            !f.isSpell && 
            !f.isCraftable && 
            (f.isExploit || f.type === 'Action' || f.type === 'Reaction' || f.type === 'Free Action' || f.cost)
        );

        // Filter: Passives (Everything else)
        const passiveList = allFeatures.filter(f => 
            !f.isSpell && 
            !f.isCraftable && 
            !activeList.includes(f)
        );
        ...
        <!-- PAGE 1: THE CHARACTER SHEET (Gameplay) -->
        <div class="print-page">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1.5fr; gap: 10px; margin-bottom: 10px;">
                
                <!-- COL 1: ATTRIBUTES & DEFENSE -->
                <div style="display:flex; flex-direction:column; gap:10px;">
                    ...
                    <div class="p-section">
                        ...
                        <div class="p-save-row">
                            <div class="p-save-box"><span class="p-save-val">${c.saves.fort >= 0 ? '+'+c.saves.fort : c.saves.fort}</span><span class="p-save-lbl">${t('save_fort').substring(0,4)}</span></div>
                            <div class="p-save-box"><span class="p-save-val">${c.saves.ref >= 0 ? '+'+c.saves.ref : c.saves.ref}</span><span class="p-save-lbl">${t('save_ref').substring(0,3)}</span></div>
                            <div class="p-save-box"><span class="p-save-val">${c.saves.will >= 0 ? '+'+c.saves.will : c.saves.will}</span><span class="p-save-lbl">${t('save_will').substring(0,4)}</span></div>
                        </div>
                    </div>
            ...
            <div class="p-section">
                ...
                <div style="column-count: 3; font-size: 8pt; gap: 10px;">
                    ${c.inventory.map(i => `
                ...
                <div style="margin-top:5px; border-top:1px solid #ccc; padding-top:2px; font-weight:bold; font-size:8pt;">
                    ${t('inv_gold')}: ${c.currency.g} | ${t('inv_silver')}: ${c.currency.s} | ${t('inv_copper')}: ${c.currency.c}
                </div>
            </div>

            <!-- NOTES (Moved to Page 1) -->
            <div class="p-section" style="flex-grow: 1;">
                <div class="p-header">${t('sheet_notes')}</div>
                <div class="p-notes-area">${c.notes || ''}</div>
            </div>
        </div>

        <!-- PAGE 2: FEATURES, SPELLS & RULES -->
        <div class="print-page">
            <div class="p-grid-2">
                
                <!-- LEFT COLUMN: ABILITIES -->
                <div class="p-col">
                    <!-- CLASS ACTIONS (New Category) -->
                    ${activeList.length > 0 ? `
                        <div class="p-section">
                            <div class="p-header">Actions & Exploits</div>
                            ${activeList.map(f => `
                                <div class="p-card exploit">
                                    <div class="p-card-top">
                                        <span class="p-card-title">${f.name}</span>
                                        <span class="p-card-cost">${f.cost || 'Action'}</span>
                                    </div>
                                    <div class="p-card-meta">${f.type} &bull; ${f.source || 'Class'}</div>
